...
**Mechanical enforcement.** `scripts/gates/pretooluse-data-guard.sh` is registered as a PreToolUse hook by `setup.sh` — in `~/.claude/settings.json` under Claude Code, and in `~/.zcode/cli/config.json` (`.hooks.events.PreToolUse`) under ZCode. Under Codex it is installed per project by `/scholar-init` (`<project>/.codex/config.toml`). `setup.sh --harness claude|codex|zcode|all` selects the target; the default auto-detects. The PostToolUse output redactor is Claude Code-only: Codex and ZCode cannot rewrite Bash output, so on those hosts Lockdown is the tier to use for restricted data. It refuses anything targeting a file whose sidecar status is `NEEDS_REVIEW:*` or `HALTED`, so even if a sub-skill forgets to check, the hook stops the unsafe access before it reaches the API. It now covers two channels that used to be open:
