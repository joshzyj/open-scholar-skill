...
- `codex-pretooluse-hook.sh` — Codex-host adapter for `pretooluse-data-guard.sh`; delegates to the guard and emits Codex's deny wire. Installed into `<proj>/.codex/config.toml` by `scripts/phases/setup-codex-hooks.sh` (via scholar-init) for codex/unknown hosts, since Codex never reads `~/.claude/settings.json`.
- `generate-lockdown-config.sh` — the lockdown tier (`/scholar-safety level lockdown`); writes the OS-sandbox read-deny on data dirs for the detected host (Claude `sandbox.filesystem.denyRead`; Codex `[permissions]` `"data"="deny"`). The only kernel-enforced tier.
...
3. **Enforcement** — `scripts/gates/pretooluse-data-guard.sh`. Intended for global registration in `~/.claude/settings.json`. Refuses unsafe reads even if a sub-skill forgets to check. **Under a Codex host** (which never reads `~/.claude/settings.json`, so the Claude hook is inert), `scholar-init` installs the *same* guard as a Codex PreToolUse hook via a `[hooks.PreToolUse]` block in `<project>/.codex/config.toml` (adapter `scripts/gates/codex-pretooluse-hook.sh`). The kernel-enforced **lockdown** tier (`/scholar-safety level lockdown`) is generated by `scripts/gates/generate-lockdown-config.sh` (Claude `sandbox.filesystem.denyRead` / Codex `[permissions]` `"data"="deny"`). Full story in `_shared/data-handling-policy.md` §9.
...
- **Privacy (C-01 / LOCAL_MODE):** traces carry aggregate metrics, verdicts, counts, and file refs ONLY — never raw data rows, verbatim quotes, or PII. **Scope:** this rule governs traces; the Evidence Ledger (`output/[slug]/evidence/*.ndjson`, per `_shared/evidence-ledger.md`) is the sanctioned home for verbatim *published-source* passages (quote-capped, excluded from replication packages) — traces reference anchors by `anchor_id`, never inline the quote, and audit tooling must not flag `evidence/` files as trace-privacy violations.
